...
module Rhales
  ...
  class RueFormatParser
    ...
    def parse_section_content(tag_name)
      ...
      scanner = StringScanner.new(@content[content_start..-1])

      # Find the closing tag position using lookahead
      if scanner.scan_until(/(?=#{Regexp.escape(closing_tag)})/)
        # scanner.pos gives us position right before closing tag
    ...
    # Add this new helper method after the other helper methods:
    def advance_to_position(target_position)
      while @position < target_position && !at_end?
        advance
      end
    end
